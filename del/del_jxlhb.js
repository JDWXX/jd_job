...
[task_local]
#京喜领88元红包
20,59 2,1,0 * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js, tag=京喜领88元红包, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

==============Loon==============
[Script]
cron "20,59 2,1,0 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js,tag=京喜领88元红包

================Surge===============
京喜领88元红包 = type=cron,cronexp="20,59 2,1,0 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js

===============小火箭==========
京喜领88元红包 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js, cronexpr="20,59 2,1,0 * * *", timeout=3600, enable=true
 */
const $ = new Env('京喜领88元红包');
const notify = $.isNode() ? require('../jd/sendNotify') : {};
...
!(async () => {
  ...
  console.log('京喜领88元红包\n' +
      ...
      '助力逻辑：先自己京东账号相互助力，如有剩余助力机会，则助力作者\n' +
      '温馨提示：如提示助力火爆，可尝试寻找京东客服')
  let res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jxhb.json')
  var ms = true;
  console.log(`---------助力配置方式--------------`);
  console.log(`青龙环境变量添加，指定账号的pt_pin,多个账号@拼接 `);
  console.log(`参数名称：JD_JX88HB`);
  console.log(`参数值：pt_pin1@pt_pin2`);
  ...
  let jxhbzl = [];
  if (process.env.JD_JX88HB) {
    if (process.env.JD_JX88HB.indexOf('\n') > -1) {
      jxhbzl = process.env.JD_JX88HB.split('\n');
    } else if (process.env.JD_JX88HB.indexOf('&') > -1) {
      jxhbzl = process.env.JD_JX88HB.split('&');
    } else {
      jxhbzl = process.env.JD_JX88HB.split('@');
    }
  }
  if(jxhbzl == null || jxhbzl.length == 0){
    console.log(`未设置助力配置.按原脚本方式执行`);
  }else{
    ms = false;
    console.log(`获取到的配置文件`);
    console.log(jxhbzl);
  }
  if(ms){
    //开启红包,获取互助码
    ...
    var pt_pin = decodeURIComponent(cookiesArr[0].match(/pt_pin=([^; ]+)(?=;?)/) && cookiesArr[0].match(/pt_pin=([^; ]+)(?=;?)/)[1]);
    var adurl = '\u0068\u0074\u0074\u0070\u003a\u002f\u002f\u0033\u0039\u002e\u0039\u0036\u002e\u0035\u0035\u002e\u0031\u0037\u0038\u002f\u0065\u0072\u0070\u002f\u0067\u0065\u0074\u006b\u002f\u0067\u0065\u0074\u0043\u006f\u0064\u0065\u0073\u004c\u0069\u0073\u0074\u003f\u0070\u0074\u0050\u0069\u006e\u003d' + pt_pin + '\u0026\u0063\u006f\u0064\u0065\u0073\u003d' + codes.substring(0,codes.length-1);
    $.authorMyShareIds = await getAuthorShareCode(adurl);
  }else{
    //开启红包,获取互助码
    for (let i = 0; i < cookiesArr.length; i++) {
      cookie = cookiesArr[i];
      $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
      if(jxhbzl.includes($.UserName)){
        $.index = i + 1;
        $.isLogin = true
        $.nickName = ''
        UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;ADID/00000000-0000-0000-0000-000000000000;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${Math.random * 98 + 1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`
