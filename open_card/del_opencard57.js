/*
1.21~1.28 年货不打烊 [gua_opencard.js]
新增开卡脚本
一次性脚本
  ...
  switch (type) {
    case 'isvObfuscator':
      ...
      case 'getSimpleActInfoVo':
        ...
        body = `activityId=dzlhkkkbblnt20220121A`;
        break;
      case 'getMyPing':
      ...
      case 'accessLogWithAD':
        ...
        let pageurl = `${domain}/drawCenter/activity?activityId=dzlhkkkbblnt20220121A&shareUuid=${$.shareUuid}`
        body = `venderId=${$.shopId || $.venderId || ''}&code=99&pin=${encodeURIComponent($.Pin)}&activityId=dzlhkkkbblnt20220121A&pageUrl=${encodeURIComponent(pageurl)}&subType=app&adSource=`
        break;
      case 'getUserInfo':
      ...
      case 'activityContent':
        ...
        body = `activityId=dzlhkkkbblnt20220121A&pin=${encodeURIComponent($.Pin)}&pinImg=${encodeURIComponent($.attrTouXiang)}&nick=${encodeURIComponent($.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid}`
        break;
      case 'drawContent':
        url = `${domain}/dingzhi/taskact/common/drawContent`;
        body = `activityId=dzlhkkkbblnt20220121A&pin=${encodeURIComponent($.Pin)}`
        break;
      case 'checkOpenCard':
      ...
      case 'info':
        ...
        body = `activityId=dzlhkkkbblnt20220121A&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}`
        break;
      case 'startDraw':
        url = `${domain}/joint/order/draw`;
        body = `activityId=dzlhkkkbblnt20220121A&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&drawType=1`
        break;
      case 'followShop':
      ...
      case '助力':
        ...
        body = `activityId=dzlhkkkbblnt20220121A&pin=${encodeURIComponent($.Pin)}&shareUuid=${$.shareUuid}`
        break;
      case 'viewVideo':
      case 'visitSku':
      case 'toShop':
      case 'addSku':
        ...
        body = `activityId=dzlhkkkbblnt20220121A&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&taskType=${taskType}&taskValue=${taskValue}`
        break;
      ...
      case '抽奖':
        ...
        body = `activityId=dzlhkkkbblnt20220121A&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.Pin)}`
        break;
      default:
        console.log(`错误${type}`);
    }
...
function getPostRequest(url, body, method="POST") {
  ...
  if(url.indexOf('https://lzdz1-isv.isvjcloud.com') > -1){
    headers["Referer"] = `https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkkkbblnt20220121A&shareUuid=${$.shareUuid}`
...
function getCk() {
  return new Promise(resolve => {
    let get = {
      url:`https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkkkbblnt20220121A&shareUuid=${$.shareUuid}`,
