...
整点跑 红包几率大点
https://u.jd.com/SLyrBKC
跳转到app 可查看助力情况
export gua_nhjRed_rebateCode="SLyrBKC"
需要助力[火力值]的账号pin值
如：【京东账号2】pin
pin1换成对应的pin值 用,分开
...
*/
let rebateCodes = 'SLyrBKC'
let rebatePin = ''

const $ = new Env('年货节红包加码');
...
async function run(type = 0){
    try{
        ...
        do{
            ...
            await getUrl1()
            $.actId = $.url2.match(/mall\/active\/([^/]+)\/index\.html/) && $.url2.match(/mall\/active\/([^/]+)\/index\.html/)[1] || `https://prodev.m.jd.com/mall/active/2UboZe4RXkJPrpkp6SkpJJgtRmod/index.html?unionActId=31137&d=SLyrBKC`
                ...
                if(Object.getOwnPropertyNames(shareCodeArr).length > s && timeH != 10 && timeH != 20){
                    for(let i in shareCodeArr || {}){
                        ...
                        if(n == s) {
                            ...
                            if(res.indexOf('上限') > -1){
                                await $.wait(parseInt(Math.random() * 4000 + 3000, 10))
                                await getCoupons('',1)
                            }
                        }
                        n++
...
function getCoupons(shareId = '',type = 1) {
    return new Promise(async resolve => {
        await requestAlgo();
        let time = Date.now()
        let body = {"platform": 4,"unionActId": "31137","actId": $.actId,"d": "SLyrBKC","unionShareId": shareId, "type": type,"eid": -1}
...
function showCoupon(shareId = '') {
    ...
    return new Promise(resolve => {
        let opts = {
            url: `https://api.m.jd.com/api?functionId=showCoupon&appid=u&_=${Date.now()}&loginType=2&body={%22actId%22:%22${$.actId}%22,%22unionActId%22:%2231137%22,%22unpl%22:%22%22,%22platform%22:4,%22unionShareId%22:%22%22,%22uiUpdateTime%22:1641456650000,%22d%22:%22SLyrBKC%22,%22eid%22:%22${$.eid}%22}&client=apple&clientVersion=8.3.6`,
            headers: {
                "Accept-Language": "zh-cn",
                "Accept-Encoding": "gzip, deflate, br",
                'Cookie': `${newCookie} ${cookie}`,
...
function shareUnionCoupon() {
    ...
    return new Promise(resolve => {
        let opts = {
            url: `https://api.m.jd.com/api?functionId=shareUnionCoupon&appid=u&_=${Date.now()}&loginType=2&body={%22unionActId%22:%2231137%22,%22actId%22:%22${$.actId}%22,%22platform%22:4,%22unionShareId%22:%22${$.shareCode}%22,%22d%22:%22SLyrBKC%22,%22supportPic%22:2,%22supportLuckyCode%22:0,%22eid%22:%22${$.eid}%22}&client=apple&clientVersion=8.3.6`,
            headers: {
                "Accept-Language": "zh-cn",
                "Accept-Encoding": "gzip, deflate, br",
                'Cookie': `${newCookie} ${cookie}`,
...
function getUrl() {
    return new Promise(resolve => {
        const options = {
            url: `https://u.jd.com/SLyrBKC${$.shareCode && "?s="+$.shareCode || ""}`,
            followRedirect:false,
            headers: {
                'Cookie': `${newCookie} ${cookie}`,
                "User-Agent": $.UA
