...
[task_local]
#穿行寻宝-膨胀内部互助
10 20 * * *  https://github.com/JDWXX/jd_job.git, tag=穿行寻宝-膨胀内部互助, enabled=true

================Loon==============
[Script]
cron "10 20 * * * " script-path=https://github.com/JDWXX/jd_job.git,tag=穿行寻宝-膨胀内部互助

===============Surge=================
穿行寻宝-膨胀内部互助 = type=cron,cronexp="10 20 * * * ",wake-system=1,timeout=3600,script-path=https://github.com/JDWXX/jd_job.git

============小火箭=========
穿行寻宝-膨胀内部互助 = type=cron,script-path=https://github.com/JDWXX/jd_job.git, cronexpr="10 20 * * * ", timeout=3600, enable=true
 */
const $ = new Env('穿行寻宝膨胀-内部互助');
        ...
        $.post(taskPostUrl("promote_pk_getExpandDetail",body), async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    if (safeGet(data)) {
                        data = JSON.parse(data);
                        if (data.code === 0 && data.data && data.data.result && data.data.result.inviteId) {
                            console.log(`\n\n 成功获取组队码\n`)
