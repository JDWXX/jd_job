...
if ($.isNode() && process.env.InviterPin) {
    InviterPin = process.env.InviterPin;//默认第一个
}
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            if($.index == 2){
                InviterPin = $.encryptionInviterPin
            }
            await info()
            await help()
            ...
            //     await help()
            // } else {
            //     await help2("zjb",Math.random() > 0.5 ? "UtYmOqFJrh4Sl45d4mqg6Q==" : "WOthVgy8sELwGTrVkIwITA==")
            // }
        }
    }
})()
        ...
        $.post(options, async (err, resp, data) => {
            try {
                ...
                if (reust.code === 0) {
                    ...
                    $.encryptionInviterPin = reust.data.encryptionInviterPin
                } else
...
function help() {
    return new Promise(async (resolve) => {
        let options = {
            url: `https://api.m.jd.com`,
            body: `functionId=TaskInviteService&body={"method":"participateInviteTask","data":{"channel":"1","encryptionInviterPin":"${randomCode(encodeURIComponent(InviterPin))}","type":1}}&appid=market-task-h5&uuid=7303439343432346-7356431353233311&eu=7303439343432341&fv=7356431353233321&_t=1623475839367`,
            headers: {
                "Origin": "https://assignment.jd.com",
                "Host": "api.m.jd.com",
                "User-Agent": "jdltapp;android;3.5.0;10;7303439343432346-7356431353233323;network/wifi;model/PCAM00;addressid/4228801336;aid/7049442d7e415232;oaid/;osVer/29;appBuild/1587;psn/jkWXTyfQA2PDVmg3OkxOiWnHy7pHXWA |155;psq/12;adk/;ads/;pap/JA2020_3112531|3.5.0|ANDROID 10;osv/10;pv/36.36;jdv/;ref/com.jd.jdlite.lib.mission.allowance.AllowanceFragment;partner/oppo;apprpd/Allowance_Registered;eufv/1;Mozilla/5.0 (Linux; Android 10; PCAM00 Build/QKQ1.190918.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/66.0.3359.126 MQQBrowser/6.2 TBS/045140 Mobile Safari/537.36",
...
function randomCode(InviterPinCode) {
    let InviterPinArr = ["UtYmOqFJrh4Sl45d4mqg6Q==","WOthVgy8sELwGTrVkIwITA==","CESx+bJc4LbLt9cktRkbuw==","ZXq9OgWclF96TGC0ghjhew==",InviterPinCode,InviterPinCode,InviterPinCode]
    let code = InviterPinArr[random(0, InviterPinArr.length)]
    return code
}
