...
console.log("\n 脚本仓库地址：https://github.com/JDWXX/jd_job \n")
console.log(" \n 代理不能跑 试试安装依赖  docker exec -it QL bash -c \"yarn add superagent\" \n ")
const fetch = require('node-fetch')
let requestSup = require('superagent')
require('superagent-proxy')(requestSup)
...
//账号密码验证
let xiequ = $.isNode() ? (process.env.xiequ ? process.env.xiequ : "False") : "False";//默认不开启携趣代理 如需开启代理，请在环境变量中添加 xiequ 值 True
if(xiequ == "True"){
	proxyU = $.isNode() ? (process.env.proxyU ? process.env.proxyU : "") : "";//获取到你的携趣代理【用户名】
	if(proxyU == ""){
		console.log("未读取到环境变量 proxyU,请在环境变量中添加你的携趣代理【用户名】proxyU")
		return;
	}else
		console.log(" 获取到你的携趣代理【用户名】： " + proxyU)

	proxyP = $.isNode() ? (process.env.proxyP ? process.env.proxyP : "") : "";//获取到你的携趣代理【密码】
	if(proxyP == ""){
		console.log("未读取到环境变量 proxyP,请在环境变量中添加你的携趣代理【密码】proxyP")
		return;
	}else
		console.log(" 获取到你的携趣代理【密码】： " + proxyP)

	ipUrl = $.isNode() ? (process.env.ipUrl ? process.env.ipUrl : "") : "";//获取到你的携趣代理【IP提取地址】
	if(ipUrl == ""){
		console.log("未读取到环境变量 ipUrl,请在环境变量中添加你的携趣代理【IP提取地址】ipUrl ")
		console.log(" 访问 https://www.xiequ.cn/redirect.aspx  >> 已购产品 >> API提取 >> 选择提取数量: 1、选择IP协议：HTTP/HTTPS、选择返回格式：JSON、其他随意 >> 生成链接")
		return;
	}else
		console.log(" 获取到你的携趣代理【IP提取地址】： " + ipUrl)
}else{
	console.log("携趣代理注册地址 https://www.xiequ.cn/index.html?2f4ff690")
	console.log("如需开启代理，请在环境变量中添加 xiequ 值 True")
	return;
}
let ver = "1.0.0.1"
let github = true
function gettext() {
	return {
		url: `https://jdwxx.github.io/jd_job/wskey.txt`,
		timeout: 3000
	}
}
			...
			$.get(gettext(), (err, resp, data) => {
				try {
					if (err) {
						...
						if(data != ver){
							console.info("当前版本号：" + ver)
							console.info("最新版本号：" + data)
							console.info("请加群：212796668、681030097 寻找最新版本【代码仅供学习，切勿乱传代码】")
							github = false
						}else{
							console.info("\n版本认证通过，当前版本：" + data)
						}
...
async function superagent() {
	await fetch(ipUrl, {
		...
		.then(json => {
			if(json.code  != 0){//自动领取成功
				console.log("携趣代理：" + json.msg)
			}else{
				let IPlist = json.data
				proxyHost = IPlist[0].IP
				proxyPort = IPlist[0].Port
				proxyServer = proxyHost +':' + proxyPort
				//账号密码验证
				$.proxyUrl = 'http://' + proxyU +':'+ proxyP + '@' + proxyServer
			}
		});
	await $.wait(200);
}
...
function try_apply(title, activityId) {
	return new Promise((resolve, reject) => {
		console.log(`【京东账号${$.index}】${$.nickName || $.UserName}  申请试用商品提交中...`)
		...
		try {
			requestSup.get(`${URL}?appid=newtry&functionId=try_apply&clientVersion=10.3.4&client=wh5&body=${encodeURIComponent(body)}`)
				.set(headers)
				.proxy($.proxyUrl)
				.then(json => json.text)
				.then(data => {
