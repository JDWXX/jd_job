...
[task_local]
#京喜工厂
10 4,7,13,18 * * * jd_dreamFactory.js, tag=京喜工厂, img-url=https://github.com/58xinian/icon/raw/master/jdgc.png, enabled=true

================Loon==============
[Script]
cron "10 4,7,13,18 * * *" script-path=jd_dreamFactory.js,tag=京喜工厂

===============Surge=================
京喜工厂 = type=cron,cronexp="10 4,7,13,18 * * *",wake-system=1,timeout=3600,script-path=jd_dreamFactory.js

============小火箭=========
京喜工厂 = type=cron,script-path=jd_dreamFactory.js, cronexpr="10 4,7,13,18 * * *", timeout=3600, enable=true
              ...
              if (safeGet(data)) {
                ...
                if (data["ret"] === 0) {
                  ...
                  if (data.factoryList && data.productionList) {
                    ...
                    $.encryptPin = data.user.encryptPin;
                    // subTitle = data.user.pin;
                    console.log(`【京东账号${$.index}（${$.UserName}）京喜工厂互助码】${data.user.encryptPin}`);
                    shareCodesdq += data.user.encryptPin + "@"
                  }
                } else {
                  $.unActive = false; //标记是否开启了京喜活动或者选购了商品进行生产
                  if (!data.factoryList) {
