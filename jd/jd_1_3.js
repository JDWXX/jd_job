...
function followShop() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=23&taskValue=1000009621&shareUuid=${$.shareUuid ? $.shareUuid : '0fee7c35965f4de18239cc28aa8c9ac6'}`
...
function getDrawRecordHasCoupon() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'ba12967ba69a4f808a98ff3042b5f09f'}&pin=${encodeURIComponent($.myPingData.secretPin)}`
...
function saveTask(taskType,taskValue) {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}&taskType=${taskType}&taskValue=${taskValue}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'ba12967ba69a4f808a98ff3042b5f09f'}`
...
async function drawContent() {
    return new Promise(resolve => {
        ...
        $.post(taskPostUrl('/dingzhi/taskact/common/drawContent', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/805745?activityId=dz2109100001616201&shareUuid=${$.shareUuid ? $.shareUuid : 'ba12967ba69a4f808a98ff3042b5f09f'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
async function startDraw(type) {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'ba12967ba69a4f808a98ff3042b5f09f'}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
        $.post(taskPostUrl('/dingzhi/shop/league/startDraw', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/805745?activityId=dz2109100001616201&shareUuid=${$.shareUuid ? $.shareUuid : 'ba12967ba69a4f808a98ff3042b5f09f'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
