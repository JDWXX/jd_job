...
开学新花young 热爱不限量
2021-8-28 ~ 2021-9-22
https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/8124226?activityId=46fcab8e90e243dcb1aa76a5c7c764d2&shareUuid=22f579c4eb10448c94d3329759d2765c
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=22f579c4eb10448c94d3329759d2765c&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await young();
            // 帮助作者，谢谢
            if(helpAu){
                await mpdz();
...
async function young() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.index === 1) {
                await task('dz/openCard/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=22f579c4eb10448c94d3329759d2765c`, 0, 1)
            } else {
                await task('dz/openCard/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=22f579c4eb10448c94d3329759d2765c`)
            }
            await task('dz/openCard/checkOpenCard', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&shareUuid=22f579c4eb10448c94d3329759d2765c&pin=${encodeURIComponent($.secretPin)}`)
            $.log("->关注店铺")
            if ($.shopTask) {
...
async function mpdz() {
    $.token = null;
    $.secretPin = null;
    await getFirstLZCKmpdz()
    await getToken();
    if($.token){
        await taskmpdz('ql/front/getFansInfo', `${JSON.stringify({"data":$.token,"source":"01","actId":"1760001","method":"/ql/front/getFansInfo","userId":""})}`, 1)
        if ($.secretPin) {
            await taskmpdz('ql/front/participantBehavior', `${JSON.stringify({"actId":"1760001","buyerNick":`${encodeURIComponent($.secretPin)}`,"behavior":"realmeinviteHelp","inviterNick":"6AB38760E008F49D94C5B6A68CBBE738DCF554EB5667BF1BB1C8657EDC5860D449336DE54E26AA8F2834B248E6398CB7A755DF4FDAE585EC3E1ABE26F3DD3CFFC956D12974FF00A045D8E31A84FE84C18A8357DE96A1F617B8AC4D64BC24B689","isSendAward":1,"target":"","channel":"app","method":"/ql/front/participantBehavior","userId":""})}`, 1);
        }
    }
}

function taskmpdz(function_id, body, isCommon = 0) {
    return new Promise(resolve => {
        $.post(taskUrlmpdz(function_id, body, isCommon), async (err, resp, data) => {
            try {
                if (err) {
                    $.log(err)
                } else {
...
function taskUrlmpdz(function_id, body, isCommon) {
    return {
        url: isCommon ? `https://mpdz-isv.isvjcloud.com/${function_id}` : `https://mpdz-isv.isvjcloud.com/ql/${function_id}`,
        headers: {
            ...
            Origin: 'https://mpdz-isv.isvjcloud.comm',
            'User-Agent': `jdapp;iPhone;9.5.4;13.6;${$.UUID};network/wifi;ADID/${$.ADID};model/iPhone10,3;addressid/0;appBuild/167668;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 13_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1`,
            Connection: 'keep-alive',
            Referer: 'https://mpdz-isv.isvjcloud.com/h5/?lng=00.000000&lat=00.000000&sid=&un_area=',
            Cookie: cookie
        },
        body: body
...
function getFirstLZCKmpdz() {
    return new Promise(resolve => {
        $.get({ url: 'https://mpdz-isv.isvjcloud.com/h5/?lng=00.000000&lat=00.000000&sid=&un_area=' }, (err, resp, data) => {
            try {
                if (err) {
                    console.log(err)
                } else {
