...
[task_local]
#东东工厂
30 6-23/4 * * * jd_jdfactory.js, tag=东东工厂, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_factory.png, enabled=true
================Loon==============
[Script]
cron "30 6-23/4 * * *" script-path=jd_jdfactory.js,tag=东东工厂
===============Surge=================
东东工厂 = type=cron,cronexp="30 6-23/4 * * *",wake-system=1,timeout=3600,script-path=jd_jdfactory.js
============小火箭=========
东东工厂 = type=cron,script-path=jd_jdfactory.js, cronexpr="30 6-23/4 * * *", timeout=3600, enable=true
 */
const $ = new Env('东东工厂');
...
const inviteCodes = ['T010-vx3Qh8e9ACjVWnYaS5kRrbA', 'T012a1nTlrK8LPBsCjVWnYaS5kRrbA', 'T0205KkcG3ZmrxWNaEWB4atuCjVWnYaS5kRrbA', 'T018a1nTlJaULfRqIBn1lgCjVWnYaS5kRrbA'];
!(async () => {
  await requireConfig();
  if (!cookiesArr[0]) {
  ...
  if(JDWXX_ZL == "JDWXX"){
    ...
    // shareCodesArr.push("T010-vx3Qh8e9ACjVWnYaS5kRrbA@T012a1nTlrK8LPBsCjVWnYaS5kRrbA")
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
            ...
            if (err) {
              ...
              if (safeGet(data)) {
                ...
                if (data.data.bizCode === 0) {
                  ...
                  $.taskVos.map((item) => {
                    if (item.taskType === 14) {
                      if(item.assistTaskDetailVo && item.assistTaskDetailVo.taskToken){
                        console.log(`【京东账号${$.index}（${$.UserName}）东东工厂互助码】${item.assistTaskDetailVo.taskToken}`);
                        shareCodesdq += item.assistTaskDetailVo.taskToken + "@"
                      }
                    }
                  });
                }
